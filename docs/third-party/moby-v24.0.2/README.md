...
- Repository copy: `server/cloud/docker-sandbox.ts`; baseline data is preserved and only a final `SCMP_ACT_ALLOW` rule for `clone`, `unshare`, `mount`, `umount2`, `pivot_root` is added.
- Modified profile, canonical compact JSON SHA-256: `5ebaddc3fd4104283573c2aa3b14e6c650c0c10735d1cc2c3a44b63d26200c7e`.
- Change date: 2026-09-25. Purpose: explicit Docker/bubblewrap compatibility, with non-root user, no capabilities, no-new-privileges, read-only root, and controlled proc mount changes; see ADR 0021.
