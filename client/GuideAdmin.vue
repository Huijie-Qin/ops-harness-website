...
<template>
  <div class="guide-admin" :class="authenticated?'admin-shell':'container admin-login-page'">
    ...
    <div v-if="entered" v-show="authenticated" class="admin-section-body"><AnalyticsManager v-if="section==='analytics' && authenticated" :view="route.view" :query="route.query" @navigate="analyticsNavigate" @error="handleError" /><ReleaseManager v-if="section==='releases'" :key="childKey" :csrf="csrf" @error="handleError" @dirty="childDirty=$event" @busy="childBusy=$event" /><KnowledgeManager v-if="section==='knowledge'" :key="childKey" :csrf="csrf" @error="handleError" @dirty="childDirty=$event" @busy="childBusy=$event" /><MediaLibrary v-if="section==='media'" :key="childKey" :csrf="csrf" @error="handleError" @busy="childBusy=$event" /></div>
