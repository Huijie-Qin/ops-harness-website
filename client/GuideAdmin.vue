...
const section=ref<'guides'|'documents'|'releases'|'media'|'knowledge'|'analytics'>('guides'),childDirty=ref(false),childBusy=ref(false),childKey=ref(0),entered=ref(false)
const sections=[{id:'guides',label:'文档编辑'},{id:'documents',label:'目录与文档管理'},{id:'releases',label:'发布包'},{id:'media',label:'图片库'},{id:'knowledge',label:'知识库'},{id:'analytics',label:'运营统计'}] as const
...
function switchSection(next:typeof section.value){if(next===section.value)return;guard(()=>{section.value=next;error.value='';notice.value=''})}
...
async function login(){await run(async()=>{const secret=password.value;password.value='';const result=await call<{csrf:string}>('/api/admin/login',{method:'POST',data:{password:secret}});csrf.value=result.csrf;authenticated.value=true;entered.value=true;await list();if(!draft.value && editableChapters.value[0])adopt((await call<{chapter:Chapter}>(`/api/admin/guides/${editableChapters.value[0].id}`)).chapter)})}
function logout(){guard(()=>void run(async()=>{await call('/api/admin/logout',{method:'POST',data:{}});authenticated.value=false;entered.value=false;childDirty.value=false;childKey.value++;section.value='guides';csrf.value='';draft.value=undefined;baseline.value='';chapters.value=[]}))}
async function editDocument(id:string){await run(async()=>{adopt((await call<{chapter:Chapter}>(`/api/admin/guides/${id}`)).chapter);section.value='guides';filter.value='';await nextTick();document.getElementById('document-editor-title')?.focus()})}
async function save(){
...
async function submitManagementDialog(){const item=managementDialog.value;if(!item||!item.value.trim()||(item.type==='move-document'&&item.value===item.groupId))return;await run(async()=>{
  if(item.type==='create-document'){
    ...
    adopt(result.chapter);await list();section.value='guides';filter.value='';notice.value='文档已创建，请编辑正文并保存。'
...
function navigation(event:MouseEvent){if(!dirty.value&&!allBusy.value)return;const link=(event.target as HTMLElement).closest('a');if(link?.href && link.target!=='_blank' && !link.hasAttribute('download')){event.preventDefault();guard(()=>{allowLeave=true;location.href=link.href})}}
...
onMounted(async()=>{document.addEventListener('click',navigation,true);window.addEventListener('beforeunload',beforeUnload);try{const result=await call<{configured:boolean;authenticated:boolean;csrf?:string}>('/api/admin/session');configured.value=result.configured;authenticated.value=result.authenticated;csrf.value=result.csrf??'';if(result.authenticated){entered.value=true;await list();if(editableChapters.value[0])adopt((await call<{chapter:Chapter}>(`/api/admin/guides/${editableChapters.value[0].id}`)).chapter)}}catch(e){handleError(e)}finally{initializing.value=false}})
onBeforeUnmount(()=>{lifetime.abort();document.removeEventListener('click',navigation,true);window.removeEventListener('beforeunload',beforeUnload);for(const [url,timer] of exports){clearTimeout(timer);URL.revokeObjectURL(url)}})
</script>
<template>
  <div class="guide-admin container">
    <div class="admin-heading"><div><span class="eyebrow text-only">官网 / 管理员</span><h1>{{authenticated?'管理发布与内容。':'管理员登录'}}</h1><p>{{authenticated?'编辑文档内容、管理阅读目录，并维护软件发布。':'通过密码认证后进入管理工作区。'}}</p></div><div class="admin-actions"><a class="button secondary" href="/guide">返回阅读</a><button v-if="authenticated" class="button secondary" :disabled="allBusy" @click="prepareSync">同步到 content</button><button v-if="authenticated" class="button secondary" :disabled="allBusy" @click="logout">退出登录</button></div></div>
    ...
    <nav v-if="authenticated" class="admin-section-tabs" aria-label="管理功能"><button v-for="item in sections" :key="item.id" :aria-pressed="section===item.id" :disabled="allBusy" @click="switchSection(item.id)">{{item.label}}</button></nav>
    <div v-if="entered" v-show="authenticated" class="admin-section-body"><AnalyticsManager v-if="section==='analytics'" @error="handleError" /><ReleaseManager v-if="section==='releases'" :key="childKey" :csrf="csrf" @error="handleError" @dirty="childDirty=$event" @busy="childBusy=$event" /><KnowledgeManager v-if="section==='knowledge'" :key="childKey" :csrf="csrf" @error="handleError" @dirty="childDirty=$event" @busy="childBusy=$event" /><MediaLibrary v-if="section==='media'" :key="childKey" :csrf="csrf" @error="handleError" @busy="childBusy=$event" /></div>
    ...
    <div v-if="authenticated && section==='guides'" class="admin-layout" :aria-busy="busy">
      <aside class="admin-sidebar editor-sidebar"><h2>选择文档</h2><label for="chapter-filter">查找文档</label><input id="chapter-filter" v-model="filter" type="search" placeholder="标题或目录名称" /><nav aria-label="可编辑文档"><button v-for="chapter in editableChapters" :key="chapter.id" :aria-current="draft?.id===chapter.id?'true':undefined" :disabled="allBusy" @click="guard(()=>load(chapter.id))"><span>{{chapter.title}}</span><small>{{chapter.directoryTitle}}</small></button></nav><p v-if="!editableChapters.length" class="admin-empty">{{filter?'没有匹配的文档。':'暂无可编辑文档，请到“目录与文档管理”中创建。'}}</p></aside>
