...
export type ReleaseManifestInput = { name: string; content: string }
export type ReleaseManifestArtifact = Pick<ReleaseArtifact, 'name' | 'sha512'> & { size?: number }
export type ReleaseManifest = ReleaseManifestInput & { version: string; platform: ReleasePlatform; files: ReleaseManifestArtifact[] }
export type ReleaseDraft = { id: string; published: boolean; version: string; platform: ReleasePlatform; title: string; notes: string[]; files: ReleaseArtifact[]; manifest?: ReleaseManifest; revision: string; createdAt: string }
...
export const maxManifestBytes = 32 * 1024
