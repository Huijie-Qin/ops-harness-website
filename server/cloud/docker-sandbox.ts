/**
 * Audited Docker seccomp baseline: Moby v24.0.2 (Apache-2.0).
...
export type DockerSandbox = 'native' | 'bubblewrap'
...
/** A fixed, reviewable compatibility mode; never accepts arbitrary Docker security options. */
export function dockerSandboxHostConfig(mode: DockerSandbox = 'native') {
  if (mode === 'native') return {}
  return {
    Privileged: false,
    CapDrop: ['ALL'],
    SecurityOpt: ['no-new-privileges=true', `seccomp=${bubblewrapSeccomp}`],
    ReadonlyRootfs: true,
    // DSH's native addon loader copies verified prebuilts here before dlopen; noexec rejects mmap.
    Tmpfs: { '/tmp': 'rw,exec,nosuid,nodev,size=128m,mode=1777' },
    MaskedPaths: ['/sys/firmware'],
    ReadonlyPaths: [],
  }
}
